namespace ConsoleApp1
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"There's {count} Pêches.");

            int MaxPasteques = 0;
            foreach (var product in products)
            {
                if (product.ProductName == "Pasteques")
                {
                    
                }
            }
            Console.WriteLine($"He {MaxPasteques} sell the most Pasteques.");
        
            // Solution
            var pastequeSales = new Dictionary<string, int>();

            foreach (var product in products)
            {
                if (product.ProductName == "Pastèques")
                {
                    if (!pastequeSales.ContainsKey(product.Producer))
                    {
                        pastequeSales[product.Producer] = 0;
                    }
                    pastequeSales[product.Producer] += product.Quantity;
                }
            }
            ...
            foreach (var sale in pastequeSales)
            {
                if (sale.Value > maxPasteques)
                {
                    maxPasteques = sale.Value;
                    maxProducer = sale.Key;
                }
            }

            Console.WriteLine($"The producer who sold the most Pastèques is {maxProducer} with {maxPasteques} Pastèques.");
        }
    }
}
